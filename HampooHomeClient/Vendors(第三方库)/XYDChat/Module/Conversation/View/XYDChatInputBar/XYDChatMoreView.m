...
#define kXYDChatTopLineBackgroundColor [UIColor colorWithRed:184/255.0f green:184/255.0f blue:184/255.0f alpha:1.0f]
...
@property (strong, nonatomic) NSMutableArray<XYDChatInputViewPlugin *> *itemViews;
...
#pragma mark - XYDChatMoreViewDelegate & XYDChatMoreViewDataSource
- (NSArray<Class> *)sortedInputViewPluginArray {
    if (_sortedInputViewPluginArray) {
        return _sortedInputViewPluginArray;
    }
    NSArray *inputViewPluginDictArray = [XYDChatInputViewPluginArray copy];
    
    NSArray<NSDictionary *> *notSortedDefaultInputViewPlugin = [inputViewPluginDictArray filteredArrayUsingPredicate:[NSPredicate predicateWithFormat:@"(XYDChatInputViewPluginTypeKey < 0)"]];
    NSArray<NSDictionary *> *notSortedCustomInputViewPlugin = [inputViewPluginDictArray filteredArrayUsingPredicate:[NSPredicate predicateWithFormat:@"(XYDChatInputViewPluginTypeKey > 0)"]];
    ...
    [notSortedDefaultInputViewPlugin enumerateObjectsUsingBlock:^(NSDictionary * _Nonnull inputViewPluginDict, NSUInteger idx, BOOL * _Nonnull stop) {
        [notSortedDefaultInputViewPluginMutableTypes addObject:inputViewPluginDict[XYDChatInputViewPluginTypeKey]];
    }];
    [notSortedCustomInputViewPlugin enumerateObjectsUsingBlock:^(NSDictionary * _Nonnull inputViewPluginDict, NSUInteger idx, BOOL * _Nonnull stop) {
        [notSortedCustomInputViewPluginMutableTypes addObject:inputViewPluginDict[XYDChatInputViewPluginTypeKey]];
    }];
    ...
    NSMutableArray *sortedInputViewPluginArray = [NSMutableArray arrayWithCapacity:[[XYDChatInputViewPluginDict allKeys] count]];
    [sortedDefaultInputViewPluginTypes enumerateObjectsUsingBlock:^(NSNumber * _Nonnull typeKey, NSUInteger idx, BOOL * _Nonnull stop) {
        [sortedInputViewPluginArray addObject:[XYDChatInputViewPluginDict objectForKey:typeKey]];
    }];
...
- (void)moreView:(XYDChatMoreView *)moreView selectIndex:(NSInteger)itemType {
    NSNumber *typeKey = @(itemType);
    id<XYDChatInputViewPluginDelegate> inputViewPlugin = [[XYDChatInputViewPluginDict objectForKey:typeKey] new];
    inputViewPlugin.inputViewRef = self.inputViewRef;
    [inputViewPlugin pluginDidClicked];
}

- (NSArray *)titlesOfMoreView:(XYDChatMoreView *)moreView {
    NSMutableArray *titles = [NSMutableArray arrayWithCapacity:[[XYDChatInputViewPluginDict allKeys] count]];
    [self.sortedInputViewPluginArray enumerateObjectsUsingBlock:^(Class _Nonnull aClass, NSUInteger idx, BOOL * _Nonnull stop) {
        id<XYDChatInputViewPluginDelegate> inputViewPlugin = [aClass new];
...
- (NSArray<UIImage *> *)imagesOfMoreView:(XYDChatMoreView *)moreView {
    NSMutableArray *images = [NSMutableArray arrayWithCapacity:[[XYDChatInputViewPluginDict allKeys] count]];
    [self.sortedInputViewPluginArray enumerateObjectsUsingBlock:^(Class _Nonnull aClass, NSUInteger idx, BOOL * _Nonnull stop) {
        id<XYDChatInputViewPluginDelegate> inputViewPlugin = [aClass new];
...
- (NSInteger)inputViewPluginTypeForItemTag:(NSInteger)tag {
    NSArray<NSNumber *> *allPlugins = [XYDChatInputViewPluginDict allKeys];
...
- (void)itemClickAction:(XYDChatInputViewPlugin *)item {
    [self moreView:self selectIndex:[self inputViewPluginTypeForItemTag:item.tag]];
}

- (void)setup {
    UIImageView *topLine = [[UIImageView alloc] init];
    topLine.backgroundColor = kXYDChatTopLineBackgroundColor;
...
- (void)setupItems {
    ...
    [self.titles enumerateObjectsUsingBlock:^(id  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        ...
        XYDChatInputViewPlugin *item = [[[XYDChatInputViewPluginDict objectForKey:typeKey] alloc] initWithFrame:CGRectMake(startX, startY, self.itemSize.width, self.itemSize.height)];
...
- (UIColor *)messageInputViewMorePanelBackgroundColor {
    ...
    _messageInputViewMorePanelBackgroundColor = [[XYDChatSettingService sharedInstance] defaultThemeColorForKey:@"MessageInputView-MorePanel-BackgroundColor"];
    return _messageInputViewMorePanelBackgroundColor;
}
