...
static XYDPropertyAttribute *attributeFromSelector(Class class, SEL selector) {
    if (class == [XYDDynamicObject class] || !class) {
        return nil;
    }
    NSDictionary *methodMap = objc_getAssociatedObject(class, methodMapKey);
    
    NSString *SELString = NSStringFromSelector(selector);
    XYDPropertyAttribute *attr = [methodMap objectForKey:SELString];
    if (attr) {
        return attr;
    } else {
...
static id getter(id self, SEL _cmd) {
    XYDPropertyAttribute *attr = attributeFromSelector([self class], _cmd);
    id result = nil;
    if (attr.isWeak) {
...
@interface XYDDynamicObject () {
    NSMutableDictionary *_localData;
}

@end

@implementation XYDDynamicObject

+(BOOL)synthesizeWithPropertyAttribute:(XYDPropertyAttribute *)attr class:(Class)class {
...
//            AVLoggerD(@"methodMap:%@", methodMap);
        ...
        for (int i = 0; i < numOfProperties; ++i) {
            ...
            XYDPropertyAttribute *attr = [[XYDPropertyAttribute alloc] initWithProperty:property];
            if (attr.isDynamic) {
...
- (NSDictionary *)_rawDictionaryWithVisitedObjects:(NSMutableSet *)visitedObjects {
    if (![visitedObjects containsObject:self]) {
        ...
        for (NSString *key in [_localData allKeys]) {
            id object = [_localData objectForKey:key];
            if ([object isKindOfClass:[XYDDynamicObject class]]) {
