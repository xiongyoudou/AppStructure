          ...
          progressBlock:(XYDChatProgressBlock)progressBlock
{
    ...
    void (^downloadComplete)(NSError *error) = ^(NSError *error) {
        @synchronized(downloadingMap) {
            ...
            for (_CallBack *c in [array copy]) {
                XYDBooleanResultBlock resultBlock = c.resultBlock;
    ...
    void (^downloadProgress)(NSInteger) = ^(NSInteger percentDone){
        @synchronized(downloadingMap) {
            NSMutableArray *array = [downloadingMap objectForKey:weakSelf.url];
            for (_CallBack *c in [array copy]) {
                XYDProgressBlock progressBlock = c.progressBlock;
                if (progressBlock) progressBlock(percentDone);
            }
        }
    };
                 ...
                 block:^(id object, NSError* error) {
                     if (error == nil) {
                         ...
                         if (file.objectId.length > 0) {
                             file.isDirty = NO;
                             [AVUtils callFileResultBlock:block XYDFile:file error:error];
                         }
                         else {
                             [AVUtils callFileResultBlock:block XYDFile:nil error:[AVErrorUtils errorWithCode:kAVErrorObjectNotFound]];
                         }
                     }
                     else {
