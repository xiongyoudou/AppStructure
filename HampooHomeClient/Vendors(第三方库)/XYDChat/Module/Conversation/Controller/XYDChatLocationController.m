...
static CGFloat const XYDChatLocationPOIListCellHeight = 40.f;

@interface XYDChatLocationController ()<UITableViewDelegate,UITableViewDataSource,MKMapViewDelegate>
...
@implementation XYDChatLocationController
...
- (void)viewDidLoad{
    ...
    [[XYDLocationManager shareManager] requestAuthorization];
    [[XYDLocationManager shareManager] startLocation];
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath {
    return XYDChatLocationPOIListCellHeight;
}
    ...
    [ser startWithCompletionHandler:^(MKLocalSearchResponse *response, NSError *error) {
        ...
        dispatch_async(dispatch_get_main_queue(),^{
            ...
            [self.tableView mas_updateConstraints:^(MASConstraintMaker *make) {
                CGFloat height = XYDChatLocationPOIListCellHeight * self.placemarks.count;
...
- (void)updateCenterLocation:(CLLocationCoordinate2D)centerCoordinate {
    ...
    [[XYDLocationManager shareManager] reverseGeocodeWithCoordinate2D:centerCoordinate success:^(NSArray *placemarks) {
