...
#import "XYDChatGifEmotionView.h"
...
@property (weak, nonatomic) XYDChatGifEmotionView *gifEmotionView; /**< 当前活跃的底部view,用来指向gifEmotionView */
...
- (void)textViewDidChange:(UITextView *)textView
    ...
    CGFloat newTextViewHeight = MAX(kTextViewFrameMinHeight, MIN(kLCCKChatBarTextViewFrameMaxHeight, textSize.height));
...
- (void)setShowType:(LCCKFunctionViewShowType)showType {
    ...
    [self showemotionView:showType == LCCKFunctionViewShowFace && self.faceButton.selected];
...
- (LCCKChatemotionView *)emotionView {
    if (!_emotionView) {
        LCCKChatemotionView *emotionView = [[LCCKChatemotionView alloc] init];
