...
{
    self = [self initWithLatitude:[CLLocation xyd_CHtoWGSlatWithX:x y:y] longitude:[CLLocation xyd_CHtoWGSlongWithX:x y:y]];
    if (self != nil) {
...
- (double)xyd_CH1903Y {
    return [CLLocation xyd_WGStoCHyWithLatitude:self.coordinate.latitude longitude:self.coordinate.longitude];
}

- (double)xyd_CH1903X {
    return [CLLocation xyd_WGStoCHxWithLatitude:self.coordinate.latitude longitude:self.coordinate.longitude];
}
...
+ (double)xyd_WGStoCHyWithLatitude:(double)lat longitude:(double)lng {
    ...
    lat = [CLLocation xyd_decToSex:lat];
    lng = [CLLocation xyd_decToSex:lng];
    
    // Converts degrees to seconds (sex)
    lat = [CLLocation xyd_degToSec:lat];
    lng = [CLLocation xyd_degToSec:lng];
