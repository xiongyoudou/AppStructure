...
CGContextRef XYDCGContextCreateARGBBitmapContext(CGSize size, BOOL opaque, CGFloat scale) {
...
CGContextRef XYDCGContextCreateGrayBitmapContext(CGSize size, CGFloat scale) {
    size_t width = ceil(size.width * scale);
    size_t height = ceil(size.height * scale);
    if (width < 1 || height < 1) return NULL;
...
CGAffineTransform XYDCGAffineTransformGetFromViews(UIView *from, UIView *to) {
    ...
    after[0] = [from xyd_convertPoint:before[0] toViewOrWindow:to];
    after[1] = [from xyd_convertPoint:before[1] toViewOrWindow:to];
    after[2] = [from xyd_convertPoint:before[2] toViewOrWindow:to];
    
    return XYDCGAffineTransformGetFromPoints(before, after);
}
