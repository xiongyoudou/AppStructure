    ...
    [resultTemp enumerateKeysAndObjectsUsingBlock: ^(id key, id obj, BOOL *stop) {
        if ([dict1 objectForKey:key]) {
            if ([obj isKindOfClass:[NSDictionary class]]) {
                NSDictionary * newVal = [[dict1 objectForKey: key] xyd_dictionaryByMergingWith: (NSDictionary *) obj];
        ...
        {
            if ([obj isKindOfClass:[NSDictionary class]]) {
                NSDictionary * newVal = [[dict2 objectForKey: key] xyd_dictionaryByMergingWith: (NSDictionary *) obj];
                [result setObject: newVal forKey: key];
            } else {
                [result setObject: obj forKey: key];
            }
...
- (NSDictionary *)xyd_dictionaryByMergingWith:(NSDictionary *)dict {
    return [[self class] xyd_dictionaryByMerging:self with: dict];
}
