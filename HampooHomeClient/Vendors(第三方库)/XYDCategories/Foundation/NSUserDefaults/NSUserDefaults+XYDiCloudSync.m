...
- (void)xyd_removeValueForKey:(NSString *)key iCloudSync:(BOOL)sync
{
    [self xyd_removeObjectForKey:key iCloudSync:sync];
}



-(void)xyd_setObject:(id)value forKey:(NSString *)defaultName iCloudSync:(BOOL)sync
{
    if (sync)
...
- (void)xyd_removeObjectForKey:(NSString *)key iCloudSync:(BOOL)sync
{
    if (sync)
