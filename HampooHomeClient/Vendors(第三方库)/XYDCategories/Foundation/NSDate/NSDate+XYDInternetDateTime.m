...
+ (NSDate *)xyd_dateFromInternetDateTimeString:(NSString *)dateString formatHint:(DateFormatHint)hint {
     // Keep dateString around a while (for thread-safety)
	NSDate *date = nil;
    if (dateString) {
        if (hint != DateFormatHintRFC3339) {
            // Try RFC822 first
            date = [NSDate xyd_dateFromRFC822String:dateString];
            if (!date) date = [NSDate xyd_dateFromRFC3339String:dateString];
        } else {
            // Try RFC3339 first
            date = [NSDate xyd_dateFromRFC3339String:dateString];
            if (!date) date = [NSDate xyd_dateFromRFC822String:dateString];
        }
    }
...
+ (NSDate *)xyd_dateFromRFC822String:(NSString *)dateString {
     // Keep dateString around a while (for thread-safety)
    NSDate *date = nil;
    if (dateString) {
        NSDateFormatter *dateFormatter = [NSDate xyd_internetDateTimeFormatter];
...
+ (NSDate *)xyd_dateFromRFC3339String:(NSString *)dateString {
     // Keep dateString around a while (for thread-safety)
    NSDate *date = nil;
    if (dateString) {
        NSDateFormatter *dateFormatter = [NSDate xyd_internetDateTimeFormatter];
