...
- (NSData *)xyd_dataByInflatingWithError:(NSError *__autoreleasing *)error
{
    ...
    [self xyd_inflate:^(NSData *toAppend) {
...
- (NSData *)xyd_dataByDeflatingWithError:(NSError *__autoreleasing *)error
{
    if (![self length]) return [self copy];
    NSMutableData *outData = [NSMutableData data];
    [self xyd_deflate:^(NSData *toAppend) {
        [outData appendData:toAppend];
    }
            error:error];
    return outData;
...
- (BOOL)xyd_inflate:(void (^)(NSData *))processBlock
          error:(NSError *__autoreleasing *)error
{
    void *libz = xyd_zlibOpen();
...
// Adapted from http://www.zlib.net/zpipe.c
- (BOOL)xyd_deflate:(void (^)(NSData *))processBlock
          error:(NSError *__autoreleasing *)error
{
    void *libz = xyd_zlibOpen();
//    int (*deflateInit2_)(z_streamp, int, int, int, int, int, const char *, int) =
...
- (BOOL)xyd_writeDeflatedToFile:(NSString *)path
                          error:(NSError *__autoreleasing *)error
{
    NSFileHandle *f = xyd_createOrOpenFileAtPath(path, error);
    if (!f) return NO;
    BOOL success = YES;
    if ([self length]) {
        success = [self xyd_deflate:
                   ^(NSData *toAppend) {
...
static NSFileHandle *xyd_createOrOpenFileAtPath(NSString *path, NSError *__autoreleasing *error)
{
