...
@implementation NSData (XYDGzip)

static void *xyd_libzOpen()
{
    static void *libz;
...
- (NSData *)xyd_gzippedDataWithCompressionLevel:(float)level
{
    if (self.length == 0 || [self xyd_isGzippedData])
    {
        return self;
    }
    
    void *libz = xyd_libzOpen();
...
- (NSData *)xyd_gzippedData
{
    return [self xyd_gzippedDataWithCompressionLevel:-1.0f];
}

- (NSData *)xyd_gunzippedData
{
    if (self.length == 0 || ![self xyd_isGzippedData])
    {
        return self;
