...
{
    return [[self class] xyd_propertyKeys];
}
+ (NSArray *)xyd_propertyKeys {
...
{
    return [[self class] xyd_propertiesInfo];
}
/**
...
+ (NSArray *)xyd_propertiesInfo
{
    ...
    for (int i = 0; i < propertyCount; i++)
    {
        [propertieArray addObject:({
            ...
            NSDictionary *dictionary = [self xyd_dictionaryWithProperty:properties[i]];
...
+ (NSArray *)xyd_propertiesWithCodeFormat
{
    ...
    NSArray *properties = [[self class] xyd_propertiesInfo];
...
//            NSString *argString = [NSString stringWithCString:arg encoding:NSUTF8StringEncoding];
            [arguments addObject:[[self class] xyd_decodeType:arg]];
        }
        
        NSString *returnTypeString =[[self class] xyd_decodeType:returnType];
        NSString *encodeString = [[self class] xyd_decodeType:encoding];
...
-(NSDictionary *)xyd_protocolList{
    return [[self class]xyd_protocolList];
}
+ (NSDictionary *)xyd_protocolList
{
...
+ (NSArray *)xyd_instanceVariable
{
    ...
    NSMutableArray *result = [NSMutableArray array];
    for (int i = 0; i < outCount; i++) {
        NSString *type = [[self class] xyd_decodeType:ivar_getTypeEncoding(ivars[i])];
...
#pragma mark -- help
+ (NSDictionary *)xyd_dictionaryWithProperty:(objc_property_t)property
{
...
+ (NSString *)xyd_decodeType:(const char *)cString
{
    ...
    } else
    {
        if ([[result substringToIndex:1] isEqualToString:@"^"]) {
            result = [NSString stringWithFormat:@"%@ *",
                      [NSString xyd_decodeType:[[result substringFromIndex:1] cStringUsingEncoding:NSUTF8StringEncoding]]];
        }
    }
    return result;
}
