...
BOOL xyd_method_swizzle(Class klass, SEL origSel, SEL altSel)
{
    if (!klass)
        return NO;
...
void xyd_method_append(Class toClass, Class fromClass, SEL selector)
{
...
void xyd_method_replace(Class toClass, Class fromClass, SEL selector)
{
    if (!toClass || !fromClass || ! selector)
        return;
...
{
    xyd_method_swizzle(self.class, originalMethod, newMethod);
}

+ (void)xyd_appendMethod:(SEL)newMethod fromClass:(Class)klass
{
    xyd_method_append(self.class, klass, newMethod);
}

+ (void)xyd_replaceMethod:(SEL)method fromClass:(Class)klass
{
    xyd_method_replace(self.class, klass, method);
}

- (BOOL)xyd_respondsToSelector:(SEL)selector untilClass:(Class)stopClass
{
    return [self.class xyd_instancesRespondToSelector:selector untilClass:stopClass];
}

- (BOOL)xyd_superRespondsToSelector:(SEL)selector
{
    return [self.superclass instancesRespondToSelector:selector];
}

- (BOOL)xyd_superRespondsToSelector:(SEL)selector untilClass:(Class)stopClass
{
    return [self.superclass xyd_instancesRespondToSelector:selector untilClass:stopClass];
}

+ (BOOL)xyd_instancesRespondToSelector:(SEL)selector untilClass:(Class)stopClass
{
