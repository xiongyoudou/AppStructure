...
NSString* const xyd_affiliateTokenKey = @"at";
NSString* const xyd_campaignTokenKey = @"ct";
NSString* const xyd_iTunesAppleString = @"itunes.apple.com";
...
- (void)xyd_presentStoreKitItemWithIdentifier:(NSInteger)itemIdentifier
{
    ...
    NSString* campaignToken = self.xyd_campaignToken ?: @"";

    NSDictionary* parameters = @{
        SKStoreProductParameterITunesItemIdentifier : @(itemIdentifier),
        xyd_affiliateTokenKey : xyd_affiliateTokenKey,
        xyd_campaignTokenKey : campaignToken,
    };

    if (self.xyd_loadingStoreKitItemBlock) {
        self.xyd_loadingStoreKitItemBlock();
    }
    [storeViewController loadProductWithParameters:parameters completionBlock:^(BOOL result, NSError* error) {
        if (self.xyd_loadedStoreKitItemBlock) {
            self.xyd_loadedStoreKitItemBlock();
        }
...
- (void)xyd_productViewControllerDidFinish:(SKStoreProductViewController*)viewController
{
...
{
    return ([URLString rangeOfString:xyd_iTunesAppleString].location != NSNotFound);
}
