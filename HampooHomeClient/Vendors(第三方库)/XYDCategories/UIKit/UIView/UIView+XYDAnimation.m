...
- (void)xyd_moveTo:(CGPoint)destination duration:(float)secs option:(UIViewAnimationOptions)option {
    [self xyd_moveTo:destination duration:secs option:option delegate:nil callback:nil];
}
...
- (void)xyd_raceTo:(CGPoint)destination withSnapBack:(BOOL)withSnapBack {
    [self xyd_raceTo:destination withSnapBack:withSnapBack delegate:nil callback:nil];
}
...
- (void)xyd_rotate:(int)degrees secs:(float)secs delegate:(id)delegate callback:(SEL)method {
    [UIView animateWithDuration:secs 
                        ...
                        options:UIViewAnimationOptionCurveLinear
                     animations:^{
                         self.transform = CGAffineTransformRotate(self.transform, xyd_radiansForDegrees(degrees));
                     }
                     completion:^(BOOL finished) { 
                         if (delegate != nil) {
#pragma clang diagnostic push
...
- (void)xyd_spinClockwise:(float)secs {
    [UIView animateWithDuration:secs/4 
                        ...
                        options:UIViewAnimationOptionCurveLinear
                     animations:^{
                         self.transform = CGAffineTransformRotate(self.transform, xyd_radiansForDegrees(90));
                     }
                     completion:^(BOOL finished) { 
                         [self xyd_spinClockwise:secs];
                     }];
}
...
- (void)xyd_spinCounterClockwise:(float)secs {
    [UIView animateWithDuration:secs/4 
                        ...
                        options:UIViewAnimationOptionCurveLinear
                     animations:^{
                         self.transform = CGAffineTransformRotate(self.transform, xyd_radiansForDegrees(270));
                     }
                     completion:^(BOOL finished) { 
                         [self xyd_spinCounterClockwise:secs];
                     }];
}
...
- (void)xyd_drainAway:(float)secs {
    self.tag = 20;
	[NSTimer scheduledTimerWithTimeInterval:secs/50 target:self selector:@selector(xyd_drainTimer:) userInfo:nil repeats:YES];
}

- (void)xyd_drainTimer:(NSTimer*)timer {
    ...
    [UIView animateWithDuration:secs/2 
                     ...
                     completion:^(BOOL finished) { 
                         [UIView animateWithDuration:secs/2 
                                          ...
                                          completion:^(BOOL finished) { 
                                              if (continuously) {
                                                  [self xyd_pulse:secs continuously:continuously];
                                              }
                                          }];
                     }];
}
