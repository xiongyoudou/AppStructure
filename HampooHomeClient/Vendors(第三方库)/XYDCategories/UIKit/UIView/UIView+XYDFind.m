...
- (id)xyd_findSuperViewWithSuperViewClass:(Class)clazz
{
    if (self == nil) {
        ...
        return [self.superview xyd_findSuperViewWithSuperViewClass:clazz];
    }
}
/**
...
- (BOOL)xyd_findAndResignFirstResponder {
    ...
    for (UIView *v in self.subviews) {
        if ([v xyd_findAndResignFirstResponder]) {
            return YES;
        }
    }
...
- (UIView *)xyd_findFirstResponder {
    ...
    for (UIView *v in self.subviews) {
        UIView *fv = [v xyd_findFirstResponder];
        if (fv) {
            return fv;
        }
    }
 ...
 */
- (UIViewController *)xyd_viewController
{
