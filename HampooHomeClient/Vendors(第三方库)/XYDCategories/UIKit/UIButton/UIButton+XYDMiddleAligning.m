...
- (UIImage *)xyd_MiddleAlignedButtonImageScaleToSize:(CGSize)size
{
...
- (void)xyd_middleAlignButtonWithSpacing:(CGFloat)spacing
{
    ...
    if (imageSize.width > ceilf(maxImageWidth)) {
        ...
        newImage = [self.imageView.image xyd_MiddleAlignedButtonImageScaleToSize:CGSizeMake(maxImageWidth, imageSize.height * ratio)];
        imageSize = newImage.size;
    }
    if (imageSize.height > ceilf(maxImageHeight)) {
        CGFloat ratio = maxImageHeight / imageSize.height;
        newImage = [self.imageView.image xyd_MiddleAlignedButtonImageScaleToSize:CGSizeMake(imageSize.width * ratio, maxImageHeight)];
        imageSize = newImage.size;
    }
    if (newImage) {
