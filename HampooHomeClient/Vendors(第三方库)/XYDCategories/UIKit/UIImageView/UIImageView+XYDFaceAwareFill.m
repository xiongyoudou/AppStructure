...
static CIDetector * _xyd_faceDetector;
...
+ (void)initialize
{
    _xyd_faceDetector = [CIDetector detectorOfType:CIDetectorTypeFace
                                       context:nil
...
- (void)xyd_faceAwareFill {
    // Safe check!
    if (self.image == nil) {
        return;
    }
    
    CGRect facesRect = [self xyd_rectWithFaces];
    if (facesRect.size.height + facesRect.size.width == 0)
        return;
    self.contentMode = UIViewContentModeTopLeft;
    [self xyd_scaleImageFocusingOnRect:facesRect];
}

- (CGRect) xyd_rectWithFaces {
    // Get a CIIImage
    ...
    CIDetector* detector = _xyd_faceDetector;
...
- (void) xyd_scaleImageFocusingOnRect:(CGRect) facesRect {
