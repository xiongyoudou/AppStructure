...
void xyd_hack_uiimageview_bf(){
    ...
    Method newSetImgMethod = class_getInstanceMethod([UIImageView class], @selector(xyd_setBetterFaceImage:));
...
- (void)xyd_faceDetect:(UIImage *)aImage
{
    ...
    dispatch_async(queue, ^{
        ...
        if (xyd_detector == nil) {
            NSDictionary  *opts = [NSDictionary dictionaryWithObject:[self xyd_fast] ? CIDetectorAccuracyLow : CIDetectorAccuracyHigh
                                                              forKey:CIDetectorAccuracy];
            xyd_detector = [CIDetector detectorOfType:CIDetectorTypeFace
                                                      context:nil
                                                      options:opts];
        }
        
        NSArray* features = [xyd_detector featuresInImage:image];
        
        if ([features count] == 0) {
            BFLog(@"no faces");
            dispatch_async(dispatch_get_main_queue(), ^{
                [[self xyd_imageLayer] removeFromSuperlayer];
            });
        } else {
            BFLog(@"succeed %lu faces", (unsigned long)[features count]);
            [self xyd_markAfterFaceDetect:features
...
-(void)xyd_markAfterFaceDetect:(NSArray *)features size:(CGSize)size{
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        CALayer *layer = [self xyd_imageLayer];
