...
- (NSArray *)xyd_getImgs
{
    ...
    for (int i = 0; i < [self xyd_nodeCountOfTag:@"img"]; i++) {
...
- (NSArray *)xyd_getOnClicks
{
    ...
    for (int i = 0; i < [self xyd_nodeCountOfTag:@"a"]; i++) {
...
{
    NSString * jsString = [NSString stringWithFormat:@"document.body.style.backgroundColor = '%@'",[color xyd_webColorString]];
...
- (void)xyd_setFontColor:(UIColor *)color withTag:(NSString *)tagName
{
    NSString *jsString = [NSString stringWithFormat:
                          ...
                          nodes[i].style.color = '%@';}", tagName, [color xyd_webColorString]];
    [self stringByEvaluatingJavaScriptFromString:jsString];
}
/// 改变指定标签的字体大小
