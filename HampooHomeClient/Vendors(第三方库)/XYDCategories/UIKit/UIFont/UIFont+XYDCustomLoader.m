 ...
 */
+ (BOOL) xyd_deviceHasFullSupportForFontCollections {
 ...
 */
+ (BOOL)xyd_registerFromURL:(NSURL *)fontURL {
...
/**
 ...
 @return Registration result
 */
+ (BOOL)xyd_registerFromCGFont:(CGFontRef)fontRef {
...
+ (NSArray *)xyd_registerFontFromURL:(NSURL *)fontURL {
    // Dictionary creation
    ...
    @synchronized(appRegisteredCustomFonts) {
        ...
        if (fontPSNames == nil) {
            ...
            // Check features
            if ([UIFont xyd_deviceHasFullSupportForFontCollections]) {
                ...
                if (fontDescriptors) {
                    ...
                    if ([fontPSNames count] > 0) {
                        ...
                        if ([UIFont xyd_registerFromURL:fontURL]) {
                    ...
                    if (loadedFont != NULL) {
                        ...
                        if ([singleFontValidExtensions containsObject:[fontURL pathExtension]]) {
                            ...
                            if ([UIFont fontWithName:fontPSNames[0] size:kSizePlaceholder] == nil) {
                                ...
                                // If registration went ok
                                if ([UIFont xyd_registerFromCGFont:loadedFont]) {
                                    // Add url to this library
                                    [appRegisteredCustomFonts setObject:fontPSNames
                                                                 forKey:fontURL];
...
+ (UIFont *) xyd_customFontWithURL:(NSURL *)fontURL size:(CGFloat)size {
    ...
    NSArray *fontPSNames = [UIFont xyd_registerFontFromURL:fontURL];
...
+ (UIFont *) xyd_customFontOfSize:(CGFloat)size withName:(NSString *)name withExtension:(NSString *)extension {
    // Get url for font resource
    NSURL *fontURL = [[[NSBundle mainBundle] URLForResource:name withExtension:extension] absoluteURL];
    
    return [UIFont xyd_customFontWithURL:fontURL size:size];
}
