...
- (CGFloat)xyd_contractionFactor
{
    id contractionFactorAssociatedObject = objc_getAssociatedObject(self, @selector(xyd_contractionFactor));
...
- (void)xyd_addBezierThroughPoints:(NSArray *)pointArray
{
    ...
    if (pointArray.count < 3) {
        switch (pointArray.count) {
            ...
            case 2:
            {
                ...
                [self addQuadCurveToPoint:point1 controlPoint:xyd_ControlPointForTheBezierCanThrough3Point(self.currentPoint, point0, point1)];
            }
                break;
            default:
                break;
    ...
    for (int i = 0; i < pointArray.count; i++) {
        ...
        if (i > 0) {
            ...
            previousCenterPoint = xyd_CenterPointOf(self.currentPoint, previousPoint);
            centerPoint = xyd_CenterPointOf(previousPoint, pointI);
            
            centerPointDistance = xyd_DistanceBetweenPoint(previousCenterPoint, centerPoint);
            
            obliqueAngle = xyd_ObliqueAngleOfStraightThrough(centerPoint, previousCenterPoint);
            
            previousControlPoint2 = CGPointMake(previousPoint.x - 0.5 * self.xyd_contractionFactor * centerPointDistance * cos(obliqueAngle), previousPoint.y - 0.5 * self.xyd_contractionFactor * centerPointDistance * sin(obliqueAngle));
            controlPoint1 = CGPointMake(previousPoint.x + 0.5 * self.xyd_contractionFactor * centerPointDistance * cos(obliqueAngle), previousPoint.y + 0.5 * self.xyd_contractionFactor * centerPointDistance * sin(obliqueAngle));
        }
...
CGFloat xyd_ObliqueAngleOfStraightThrough(CGPoint point1, CGPoint point2)   //  [-π/2, 3π/2)
{
...
CGPoint xyd_ControlPointForTheBezierCanThrough3Point(CGPoint point1, CGPoint point2, CGPoint point3)
{
    return CGPointMake(2 * point2.x - (point1.x + point3.x) / 2, 2 * point2.y - (point1.y + point3.y) / 2);
}

CGFloat xyd_DistanceBetweenPoint(CGPoint point1, CGPoint point2)
{
    return sqrt((point1.x - point2.x) * (point1.x - point2.x) + (point1.y - point2.y) * (point1.y - point2.y));
}

CGPoint xyd_CenterPointOf(CGPoint point1, CGPoint point2)
{
