...
#import "UIImage+XYDPDF.h"

@implementation UIImage (XYDPDF)

+ (UIImage *)imageWithPDF:(id)dataOrPath {
    return [self xyd_imageWithPDF:dataOrPath resize:NO size:CGSizeZero];
}

+ (UIImage *)imageWithPDF:(id)dataOrPath size:(CGSize)size {
    return [self xyd_imageWithPDF:dataOrPath resize:YES size:size];
}

+ (UIImage *)xyd_imageWithPDF:(id)dataOrPath resize:(BOOL)resize size:(CGSize)size {
