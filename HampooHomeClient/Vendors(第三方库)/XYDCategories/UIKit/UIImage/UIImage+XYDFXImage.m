...
- (UIImage *)xyd_imageScaledToFitSize:(CGSize)size
{
    //calculate rect
    CGFloat aspect = self.size.width / self.size.height;
    if (size.width / aspect <= size.height)
    {
        return [self xyd_imageScaledToSize:CGSizeMake(size.width, size.width / aspect)];
    }
    else
    {
        return [self xyd_imageScaledToSize:CGSizeMake(size.height * aspect, size.height)];
    }
}

- (UIImage *)xyd_imageScaledToFillSize:(CGSize)size
{
    if (CGSizeEqualToSize(self.size, size))
    {
        return self;
...
- (UIImage *)xyd_imageCroppedAndScaledToSize:(CGSize)size
...
- (UIImage *)xyd_reflectedImageWithScale:(CGFloat)scale
{
	...
	//clip to gradient
	CGContextClipToMask(context, bounds, [[self class] xyd_gradientMask]);
...
- (UIImage *)xyd_imageWithReflectionWithScale:(CGFloat)scale gap:(CGFloat)gap alpha:(CGFloat)alpha
{
    //get reflected image
    UIImage *reflection = [self xyd_reflectedImageWithScale:scale];
...
- (UIImage *)xyd_imageWithShadowColor:(UIColor *)color offset:(CGSize)offset blur:(CGFloat)blur
{
    //get size
...
- (UIImage *)xyd_imageWithCornerRadius:(CGFloat)radius
{
    //create drawing context
...
- (UIImage *)xyd_imageWithAlpha:(CGFloat)alpha
{
    //create drawing context
	UIGraphicsBeginImageContextWithOptions(self.size, NO, 0.0f);
...
- (UIImage *)xyd_imageWithMask:(UIImage *)maskImage;
{
    //create drawing context
    UIGraphicsBeginImageContextWithOptions(self.size, NO, 0.0f);
    CGContextRef context = UIGraphicsGetCurrentContext();
...
- (UIImage *)xyd_maskImageFromImageAlpha
{
    //get dimensions
