...
+ (NSData *)xyd_compressImage:(UIImage *)image toMaxLength:(NSInteger)maxLength maxWidth:(NSInteger)maxWidth{
    NSAssert(maxLength > 0, @"图片的大小必须大于 0");
    NSAssert(maxWidth > 0, @"图片的最大边长必须大于 0");
    
    CGSize newSize = [self xyd_scaleImage:image withLength:maxWidth];
    UIImage *newImage = [self xyd_resizeImage:image withNewSize:newSize];
...
+ (UIImage *)xyd_resizeImage:(UIImage *) image withNewSize:(CGSize) newSize{
...
+ (CGSize)xyd_scaleImage:(UIImage *) image withLength:(CGFloat) imageLength{
