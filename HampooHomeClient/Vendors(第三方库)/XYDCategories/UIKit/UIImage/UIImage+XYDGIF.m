...
+ (UIImage *)xyd_animatedGIFWithData:(NSData *)data {
    ...
    if (count <= 1) {
        ...
        for (size_t i = 0; i < count; i++) {
            ...
            duration += [self xyd_frameDurationAtIndex:i source:source];
...
+ (float)xyd_frameDurationAtIndex:(NSUInteger)index source:(CGImageSourceRef)source {
...
+ (UIImage *)xyd_animatedGIFNamed:(NSString *)name {
    ...
    if (scale > 1.0f) {
        ...
        if (data) {
            return [UIImage xyd_animatedGIFWithData:data];
        }
