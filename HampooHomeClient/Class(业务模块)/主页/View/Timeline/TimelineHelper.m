...
+ (UIImage *)imageWithPath:(NSString *)path {
    ...
    if (image) return image;
    if (path.xyd_pathScale == 1) {
        // 查找 @2x @3x 的图片
        NSArray *scales = [NSBundle preferredScales];
        for (NSNumber *scale in scales) {
            image = [UIImage imageWithContentsOfFile:[path xyd_stringByAppendingPathScale:scale.floatValue]];
            if (image) break;
        }
    } else {
        image = [UIImage imageWithContentsOfFile:path];
...
+ (YYWebImageManager *)avatarImageManager {
    ...
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        NSString *path = [kCachesPath stringByAppendingPathComponent:@"weibo.avatar"];
        YYImageCache *cache = [[YYImageCache alloc] initWithPath:path];
        manager = [[YYWebImageManager alloc] initWithCache:cache queue:[YYWebImageManager sharedManager].queue];
        manager.sharedTransformBlock = ^(UIImage *image, NSURL *url) {
            if (!image) return image;
            return [image xyd_imageByRoundCornerRadius:100]; // a large value
        };
    });
    return manager;
}
...
+ (NSArray<TLEmoticonGroup *> *)emoticonGroups {
    ...
    dispatch_once(&onceToken, ^{
        ...
        groups = (NSMutableArray *)[NSArray yy_modelArrayWithClass:[TLEmoticonGroup class] json:packages];
        
        NSMutableDictionary *groupDic = [NSMutableDictionary new];
        for (int i = 0, max = (int)groups.count; i < max; i++) {
            TLEmoticonGroup *group = groups[i];
        ...
        for (NSString *path in additionals) {
            TLEmoticonGroup *group = groupDic[path];
            if (!group) continue;
            ...
            TLEmoticonGroup *addGroup = [TLEmoticonGroup yy_modelWithJSON:infoJSON];
            if (addGroup.emoticons.count) {
                for (TLEmoticon *emoticon in addGroup.emoticons) {
